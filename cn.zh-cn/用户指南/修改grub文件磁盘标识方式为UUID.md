...
-   以Ubuntu 14.04为例，执行**blkid**命令获取root分区对应的UUID并记录下来，编辑“/boot/grub/grub.cfg”文件，使用root分区的UUID来配置boot项。如果root分区已经使用UUID形式则不需要修改。具体操作方法如下：
    1.  使用root用户登录云服务器。
...
-   以CentOS 7.0为例，执行**blkid**命令获取root分区对应的UUID并记录下来，编辑“/boot/grub2/grub.cfg”文件，使用root分区的UUID来配置boot项。如果root分区已经使用UUID形式则不需要修改。
    1.  使用root用户登录云服务器。
    2.  执行以下命令，列出当前系统中所有已挂载文件系统的类型以及对应设备的UUID 。
