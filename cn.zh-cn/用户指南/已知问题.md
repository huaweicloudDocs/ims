...
## Windows Server 2008 DHCP租约设置过长（如一年），可能导致网络不通的问题<a name="section14710182124011"></a>

**问题描述：**

Windows Server 2008操作系统的DHCP Client服务存在已知问题，当租约设置过长（比如一年）会导致您的实例无法自动续约IP。当第一次的租约到期时会导致实例网络不通。

**涉及镜像：**

Windows Server 2008公共镜像及私有镜像

**解决方案：**

1.  调整实例所在的子网DHCP租约为1天。
2.  执行以下命令，使租约设置生效。
...
<td class="cellrowborder" valign="top" width="50%" headers="mcps1.1.3.1.2 "><p id="p4890357191012"><a name="p4890357191012"></a><a name="p4890357191012"></a>Ubuntu 16.04：4.4.0-151-generic</p>
<p id="p185234241786"><a name="p185234241786"></a><a name="p185234241786"></a>Ubuntu 18.04：4.15.0-52-generic</p>
</td>
</tr>
